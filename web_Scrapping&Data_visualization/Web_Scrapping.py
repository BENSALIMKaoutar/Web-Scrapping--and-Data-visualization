...
url1='https://www.babelio.com/decouvrir.php'               # page contenant des livres rassemblés par catégorie
html1=requests.get(url1).content
sel1=Selector(text=html1)
locale.setlocale(locale.LC_ALL, 'nl_NL')
a='//*[@id="page_corps"]/div/div[4]/div[4]/div[*]/a/@href' # liens vers chaque catégorie
...
for i in range(4,len(urrl)-1):
    ...
    url3=site+u[0]                                         # lien vers les livres populaires d'une catégorie déterminée
    html3=requests.get(url3).content
    sel3=Selector(text=html3)
    dd='//*[@id="page"]/div[2]/div[*]/a[1]/h2/text()'
    Title=sel3.xpath(dd).extract()                         # Extraire les titres des livres
    f='//*[@id="page"]/div[2]/div[*]/a[2]/h3/text()'
    Author=sel3.xpath(f).extract()                         # Extraire les noms des auteurs
    refpath='//*[@id="page"]/div[2]/div[*]/a[1]/@href'
    refRating=sel3.xpath(refpath).extract()                # Les liens vers la description d'un livre afin d'extraire son Rating at la note
    Rating=[]
    Note=[]
    cpt=0
    for j in range(len(refRating)):
        ...
        if len(q1)==0 or len(q2)==0 :                      # Nettoyer les données
            Title.pop(j-cpt)
            Author.pop(j-cpt)
            cpt+=1
...
#ajouter une colnne "genre" et concatener tous les dataframes en une seule
